...
///
/// Third, the fetch itself. Callers pass [`crate::http::public_client`], whose
/// resolver and redirect policy refuse private addresses at every hop, and
/// [`resolve_and_check`] refuses them once more before the request so the
/// error is a sentence rather than a transport failure,
...
async fn resolve_and_check(host: &str, port: u16, allow_loopback: bool) -> CommandResult<()> {
    ...
    for addr in addrs {
        saw_any = true;
        if crate::http::is_forbidden_address(addr.ip(), allow_loopback) {
            return Err(CommandError::new(
                codes::FORBIDDEN,
...
mod tests {
    ...
    fn loopback_and_private_addresses_are_forbidden() {
        ...
        for addr in forbidden {
            ...
            assert!(crate::http::is_forbidden_address(ip, false), "{addr} should be forbidden");
        }
    }
    ...
    fn the_loopback_override_only_ever_lifts_loopback_itself() {
        ...
        assert!(!crate::http::is_forbidden_address(loopback, true));
        let private: std::net::IpAddr = "10.0.0.1".parse().unwrap();
        assert!(
            crate::http::is_forbidden_address(private, true),
