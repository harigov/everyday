...
//!
//! Connect, sync every mailbox, hold `IDLE` (or poll, on a server without
//! it) until something changes or [`POLL_INTERVAL`] elapses, and do it all
//! again -- for as long as the supervisor keeps this key alive. See
//! `crate::supervisor`'s module doc for the restart-with-backoff and
//! stop-on-lock machinery this task's `Result` and `Ok(Outcome::Done)`
//! answers drive.
//!
...
/// IMAP. What [`crate::mailsync::wiring::ensure_account_task`] registers
/// with the supervisor.
pub async fn run_account(
    ...
    run_account_with(svc, vault, account_id, stop, connect_over_imap).await
}
...
/// obtained -- see the module docs.
pub async fn run_account_with<S, C, Fut>(
    svc: Arc<Service>,
    vault: Arc<Vault>,
    account_id: AccountId,
    ...
    Fut: Future<Output = everyday_mail::session::Result<S>>,
{
    ...
    loop {
        ...
        // supervisor's stop signal (and a `sync_account` nudge) against the
        // idle call, dropping the latter (and, with it, the connection) if
        // either wins, which is an entirely ordinary way for an IMAP
        // session to end.
