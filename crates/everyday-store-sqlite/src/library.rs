...
//!
//! Optional on the same terms as the task and calendar domains. What stays
//! in the clear here is only what an index is built from — which shelf, what
//! status, your rating, whether you starred it, the year, the finish date,
//! and a log row's date and event. Everything a person would recognise is
//! sealed: titles, creators, summaries, notes, tags, addresses, cover
//! addresses, and the *names of the shelves themselves*.
//!
...
//! every item in the vault, and what somebody holding the database file
//! learns is that eleven things were finished in March and never what any of
//! them were.
//!
...
use crate::{SqliteStore, date_str, to_us};
...
impl LibraryStore for SqliteStore {
    ...
    fn list_kinds(&self) -> Result<Vec<Kind>> {
        let conn = self.conn();
        let mut stmt = conn
            .prepare("SELECT id, data FROM kinds ORDER BY sort_order, created_us")
    ...
    fn get_kind(&self, id: KindId) -> Result<Kind> {
        ...
        let sealed: Option<Vec<u8>> = conn
            .query_row("SELECT data FROM kinds WHERE id = ?1", params![id.to_string()], |r| {
                r.get(0)
            })
            .optional()
            .map_err(Error::backend)?;
        drop(conn);
        let sealed = sealed.ok_or_else(|| Error::not_found("kind", id))?;
    ...
    fn delete_kind(&self, id: KindId) -> Result<()> {
        // The cascade would happen by foreign key -- but only if the pragma
        // is on, which `open` sets and which a future refactor could quietly
        // turn off. Three indexed statements do not depend on a connection
        // setting staying put, and the log has to go through the items to be
        // reached at all.
        ...
        .map_err(Error::backend)?;
        tx.execute("DELETE FROM items WHERE kind_id = ?1", params![id.to_string()])
            .map_err(Error::backend)?;
        tx.execute("DELETE FROM kinds WHERE id = ?1", params![id.to_string()])
            .map_err(Error::backend)?;
    ...
    fn list_items(&self, query: &ItemQuery) -> Result<Vec<Item>> {
        ...
        if let Some(floor) = query.rating_at_least {
            ...
            args.push(Box::new(i64::from(floor)));
    ...
    fn get_item(&self, id: ItemId) -> Result<Item> {
        let conn = self.conn();
        let sealed: Option<Vec<u8>> = conn
            .query_row("SELECT data FROM items WHERE id = ?1", params![id.to_string()], |r| {
                r.get(0)
            })
            .optional()
            .map_err(Error::backend)?;
        drop(conn);
        let sealed = sealed.ok_or_else(|| Error::not_found("item", id))?;
    ...
    fn delete_item(&self, id: ItemId) -> Result<()> {
        let mut conn = self.conn();
        let tx = conn.transaction().map_err(Error::backend)?;
        tx.execute("DELETE FROM logs WHERE item_id = ?1", params![id.to_string()])
            .map_err(Error::backend)?;
    ...
    fn list_logs(&self, query: &LogQuery) -> Result<Vec<LogEntry>> {
        ...
        if let Some(item) = query.item_id {
            args.push(Box::new(item.to_string()));
        ...
        if !query.events.is_empty() {
            let holes: Vec<String> = query
                ...
                .iter()
                .map(|e| {
                    args.push(Box::new(e.as_str().to_string()));
                    format!("?{}", args.len())
                })
                .collect();
    ...
    fn get_log(&self, id: LogId) -> Result<LogEntry> {
        let conn = self.conn();
        let sealed: Option<Vec<u8>> = conn
            .query_row("SELECT data FROM logs WHERE id = ?1", params![id.to_string()], |r| r.get(0))
            .optional()
            .map_err(Error::backend)?;
        drop(conn);
        let sealed = sealed.ok_or_else(|| Error::not_found("log", id))?;
    ...
    fn delete_log(&self, id: LogId) -> Result<()> {
        let conn = self.conn();
        conn.execute("DELETE FROM logs WHERE id = ?1", params![id.to_string()])
            .map_err(Error::backend)?;
        Ok(())
    }
}
