...
fn a_dream_routine_cannot_be_deleted_through_a_proposal() {
    ...
    // Saved directly (bypassing the guard the routines file itself will
    // enforce) so the vault-side check in `accept_proposal` is what is
    // actually exercised here.
    let mut dream = Routine::new("Nightly", "Read yesterday.", Trigger::Manual);
    dream.kind = RoutineKind::Dream { scope: DreamScope::Day };
    vault.save_routine(&dream).unwrap();
