...
pub trait MailStore: Send + Sync {
    ...
    /// One message by its own id -- what composing a reply or a forward
    /// reads to fill in a subject, quoted body and recipients from, and what
    /// the outbox executor's `Lookups` implementation reads the parent of a
    /// reply from. Distinct from [`MailStore::message_by_uid`], which is
    /// keyed by where the sync engine last saw a message rather than by the
