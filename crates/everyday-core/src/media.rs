//! Serving attachment bytes to a webview, and knowing what they are.
//!
//! The desktop shell answers `everyday://` requests with these, and a mobile
//! shell would answer identically. The logic lives here, in the crate with
//! no UI and no storage backend, for three reasons: both front ends need
//! exactly the same behaviour, this way it is testable on a machine that
//! cannot build a webview at all, and it is what `everyday-transfer` sniffs a
//! blob's type with on the way into an archive -- a thing that has nothing
//! to do with a vault and should not have had to depend on one to ask it.
