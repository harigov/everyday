...
impl MailSession for ImapSession {
    ...
    async fn move_to(&mut self, uids: &UidSet, mailbox: &str) -> Result<()> {
        ...
        // named; without UIDPLUS the fallback is a plain `EXPUNGE`, which
        // removes *every* `\Deleted` message in the mailbox, not only these
        // — an accepted risk noted in `docs/plans/mail.md`'s risk table,
        // because a server with neither MOVE nor UIDPLUS in 2026 is rare
        // enough that losing another client's pending deletions on it is
        // an acceptable trade for not needing a third code path.
