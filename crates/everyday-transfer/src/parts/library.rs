...
impl Portable for LibraryPart {
    ...
    fn export(&self, store: &dyn JournalStore, out: &mut Files<'_>, opts: &Options) -> Result<()> {
        ...
        for kind in &kinds {
            ...
            let mut stem = safe_name(&kind.name);
            while taken.contains(&stem) {
                stem = format!("{stem}-{}", kind.id.short());
            }
            ...
            shelves.row(&[
                ...
                kind.fields
                    .iter()
                    .map(|f| format!("{}:{}:{}", f.key, f.label, field_type(f)))
                    .collect::<Vec<_>>()
                    .join("; "),
    ...
    fn import(&self, store: &dyn JournalStore, src: &Part<'_>, mode: Mode) -> Result<Report> {
        ...
        let mut by_file: BTreeMap<String, KindId> = BTreeMap::new();
        if let Some(text) = src.text(SHELVES) {
            for row in Table::parse(text).rows() {
                match read_shelf(library, &row, mode) {
                    Ok((id, existed)) => {
                        by_file.insert(row.get("file").to_string(), id);
                        report.count(existed, mode);
                    }
                    Err(e) => report.problem(SHELVES, e),
                }
            }
        }
        ...
        for (name, text) in src.documents(".csv") {
            ...
            let kind = match by_file.get(name) {
                Some(id) => *id,
                None => match shelf_named(library, name.trim_end_matches(".csv")) {
                    Ok(id) => {
            ...
            for row in table.rows() {
                match read_item(store, library, src, &row, kind, mode) {
        ...
        if let Some(text) = src.text(LOG) {
            for row in Table::parse(text).rows() {
                match read_log(library, &row, mode) {
                    Ok(existed) => report.count(existed, mode),
...
fn common_row(item: &Item, names: &BTreeMap<KindId, String>, cover: &str) -> Vec<String> {
    vec![
        ...
        status_name(item.status).to_string(),
...
fn read_shelf(
    ...
    let mut kind = existing.clone().unwrap_or_else(|| Kind::new(&slug, name, row.get("singular")));
    kind.id = id;
...
fn parse_field(spec: &str) -> Option<everyday_core::library::FieldDef> {
    ...
    Some(everyday_core::library::FieldDef {
        ...
        field_type: match parts.next().unwrap_or("text").trim() {
            "multiline" => everyday_core::library::FieldType::Multiline,
            "number" => everyday_core::library::FieldType::Number,
            "date" => everyday_core::library::FieldType::Date,
            "url" => everyday_core::library::FieldType::Url,
            _ => everyday_core::library::FieldType::Text,
        },
        placeholder: String::new(),
    })
}

fn field_type(field: &everyday_core::library::FieldDef) -> &'static str {
    match field.field_type {
        everyday_core::library::FieldType::Text => "text",
        everyday_core::library::FieldType::Multiline => "multiline",
        everyday_core::library::FieldType::Number => "number",
        everyday_core::library::FieldType::Date => "date",
        everyday_core::library::FieldType::Url => "url",
    }
}
...
fn read_item(
    ...
    let mut item = existing.unwrap_or_else(|| Item::new(kind_id, row.get("title")));
    item.id = id;
    ...
    item.status = status(row.get("status")).unwrap_or(item.status);
...
fn read_log(library: &dyn LibraryStore, row: &crate::text::Row<'_>, mode: Mode) -> Result<bool> {
    ...
    let mut log = existing.unwrap_or_else(|| LogEntry {
        id,
        item_id,
    ...
    log.id = id;
    log.item_id = item_id;
    log.event = log_event_from(row.get("event")).unwrap_or(log.event);
...
fn status(name: &str) -> Option<ItemStatus> {
    match name.trim().to_ascii_lowercase().as_str() {
        "wishlist" | "want" | "to-read" => Some(ItemStatus::Wishlist),
        "active" | "reading" | "currently-reading" => Some(ItemStatus::Active),
        "paused" => Some(ItemStatus::Paused),
        "done" | "read" | "finished" => Some(ItemStatus::Done),
        "abandoned" | "dnf" => Some(ItemStatus::Abandoned),
        _ => None,
    }
}

fn log_event(event: LogEvent) -> &'static str {
    match event {
...
fn log_event_from(name: &str) -> Option<LogEvent> {
    match name.trim().to_ascii_lowercase().as_str() {
        "started" => Some(LogEvent::Started),
        "progress" => Some(LogEvent::Progress),
        "finished" => Some(LogEvent::Finished),
        "revisited" => Some(LogEvent::Revisited),
        "note" => Some(LogEvent::Note),
        "stopped" => Some(LogEvent::Stopped),
        _ => None,
    }
}
