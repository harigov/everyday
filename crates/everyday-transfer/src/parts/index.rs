...
impl Namer {
    ...
    ///
    /// `retry` is allowed to ignore `n`, and three of the four callers do:
    /// their suffix is the record's own short id, which is already unique and
    /// has nothing to count. That makes the obvious loop a hang rather than a
    /// long wait -- the same string, tested against the same set, for ever --
    /// and it takes only a title that happens to read like another record's
    /// suffixed name to reach it. So a candidate that repeats itself is taken
    /// as "this rule has nothing more to offer" and counted from there,
    /// keeping the part's chosen shape as the stem.
    pub fn unique(&mut self, base: &str, mut retry: impl FnMut(&str, u32) -> String) -> String {
        let mut name = base.to_string();
        let mut n = 2;
        let mut last: Option<String> = None;
        while self.taken.contains(&name) {
            let candidate = retry(base, n);
            name = match &last {
                Some(previous) if *previous == candidate => format!("{candidate}-{n}"),
                _ => candidate.clone(),
            };
            last = Some(candidate);
            n += 1;
...
mod tests {
    ...
    /// A rule that does not count still has to terminate.
    ///
    /// The three parts whose suffix is a record's short id hand `unique` a
    /// closure that ignores `n` entirely. Nothing stops two records reducing
    /// to the same stem *and* one of them already owning the suffixed form --
    /// a shelf actually called "Books-9f2a1c0d" is a legal shelf -- and when
    /// that happened the export stopped responding rather than writing a
    /// second file.
    #[test]
    fn a_suffix_that_does_not_count_still_gets_out_of_the_way() {
        let mut namer = Namer::new();
        assert_eq!(namer.unique("Books", |b, _| format!("{b}-9f2a")), "Books");
        assert_eq!(namer.unique("Books", |b, _| format!("{b}-9f2a")), "Books-9f2a");
        assert_eq!(namer.unique("Books", |b, _| format!("{b}-9f2a")), "Books-9f2a-3");
        assert_eq!(namer.unique("Books", |b, _| format!("{b}-9f2a")), "Books-9f2a-4");
    }
