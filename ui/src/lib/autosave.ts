...
// rather than sidestepping it: `errors.ts` has no static import of
// `state.svelte.ts` at all -- the handful of its functions that need `app`
// reach it through a dynamic `import()` at the point they are called, not at
// the top of the file -- so loading `isLocked` here never touches `api.ts`,
// the vault, or anything that needs a `window` to exist. That is also what
// keeps this module loadable, alone, by its own test.
