<script lang="ts">
  ...
  const heading = $derived(
    app.showStarredOnly ? 'Starred' : (app.journal?.name ?? 'All entries'),
  )
...
<section class="list">
  ...
  <div class="scroll rows">
    {#if app.query.trim()}
      ...
      {:else}
        <div class="grouphead"><span class="eyebrow">{plural(app.results.length, 'result')}</span></div>
...
<style>
  ...
  .search {
    ...
    font-size: var(--text-base); user-select: text;
    transition: border-color var(--fast) var(--ease), background var(--fast) var(--ease);
  }
  .search::placeholder { color: var(--fg-faint); }
  .search::-webkit-search-cancel-button { -webkit-appearance: none; }
  .search:focus { outline: none; border-color: var(--accent); background: var(--bg-raised); }

  .rows { flex: 1; padding-bottom: var(--sp-4); }

  .grouphead { padding: var(--sp-4) var(--sp-4) var(--sp-2); }
  ...
  .bar {
    ...
    width: 2px; border-radius: 0 2px 2px 0; opacity: 0;
    transition: opacity var(--fast) var(--ease);
  }
  .row.sel .bar { opacity: 1; }

  .cal {
    display: flex; flex-direction: column; align-items: center;
    width: 26px; flex: none; padding-top: 1px;
  }
  .dow {
  ...
  .dom {
    font-size: var(--text-md); font-weight: 550; line-height: 1.1;
    color: var(--fg-muted); font-variant-numeric: tabular-nums lining-nums;
  }
  .row.sel .dom { color: var(--fg); }
  ...
  .excerpt {
    margin-top: 2px;
    font-size: var(--text-sm); line-height: var(--leading-normal); color: var(--fg-subtle);
  ...
  .note { padding: var(--sp-6) var(--sp-4); color: var(--fg-subtle); font-size: var(--text-sm); text-align: center; }
  .blank { padding: var(--sp-10) var(--sp-4); text-align: center; }
  .blank p { color: var(--fg-subtle); margin-bottom: var(--sp-4); }
</style>
