...
>
  ...
  <div class="answers" class:open={choosing}>
    <button
      class="yes"
      disabled={busy}
      onclick={accept}
...
<style>
  ...
  /* A block on the time grid is often one lane of several, and two buttons
     side by side would fill it and hide what it is. So the title has the
     whole width, and the answers float over its corner only while the
     pointer or the keyboard is on the block -- or while its reasons are
     open. */
  /* Stacked, the way a real block on the grid reads: time, then title. */
  .compact .caption,
  .compact .open {
    display: flex;
    flex-direction: column;
  ...
  .ghost.compact:hover .answers,
  .ghost.compact:focus-within .answers,
  .compact .answers.open {
    opacity: 1;
    pointer-events: auto;
  }
