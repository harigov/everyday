...
- **Later.** The rail's confirmation card gains a third button beside
  confirm and decline that parks the call as a proposal — `ConfirmGate`
  builds the record via `build_*` and answers the model *proposed*, the
  way drafting mode does.
- **Unattended destructive calls become proposals.** A custom routine with
  nobody watching currently declines a delete on the spot; with
  `confirm_destructive` on it can draft a `Delete` instead, and the run's
  report says so. Behind its own sentence in Settings, because a routine
  that leaves deletions waiting is a different promise from one that
  refuses them.
- **A "Plan for tomorrow" card** on the Overview, drawing the pending
  block and task proposals for the next day in one place, for the person
  who wants to answer them all before bed.
